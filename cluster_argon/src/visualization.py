...
def plot_caloric_curve(target_temp, total_energy, potential_energy,
    ...
    # Subsample the data
    subsample = 2
    target_temp = target_temp[::subsample]
    total_energy = total_energy[::subsample]
    potential_energy = potential_energy[::subsample]

    fig, axes = plt.subplots(2, 1, figsize=(8, 11), sharex=True)

    ax1 = axes[0]
    #ax1.plot(target_temp, total_energy, color='tab:green', alpha=0.2, lw=0.5)
    ax1.scatter(target_temp, total_energy, color='tab:green', alpha=0.5, s=0.1,
            label='E_tot')
    ...
    ax2 = axes[1]
    #ax2.plot(target_temp, potential_energy, color='tab:blue', alpha=0.2, lw=0.5)
    ax2.scatter(target_temp, potential_energy, color='tab:blue', alpha=0.5, s=0.1,
            label='E_pot')
    ...
    # Set major ticks every 10 units and minor ticks every 2 units
    ax2.set_xticks(np.arange(min(target_temp), max(target_temp)+1, 10))  # Major ticks
    ax2.set_xticks(np.arange(min(target_temp), max(target_temp)+1, 2), minor=True)  # Minor ticks
