...
from lj_potential import compute_forces
...
def velocity_verlet_step(positions:  np.ndarray,
    ...
    acc = forces * (FORCE_CONV / mass_amu)                           # [Å/fs²]

    # Step 1: Update positions
    # r(t+dt) = r(t) + v(t)·dt + ½·a(t)·dt²
    new_positions = positions + velocities * dt_fs + 0.5 * acc * dt_fs**2

    # Step 2: Evaluate forces at updated positions
    new_forces = compute_forces(epsilon_ev, sigma_ang, new_positions) # [eV/Å]
    new_acc    = new_forces * (FORCE_CONV / mass_amu)                 # [Å/fs²]
